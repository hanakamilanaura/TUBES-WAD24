...
@section('content')
    <div class="flex items-center mb-4">
        ...
        <h1 class="text-xl font-extrabold text-gray-900">Shift Management</h1>
    </div>
    ...
    <div class="mb-4">
        <a href="{{ route('shift.create') }}" class="bg-blue-500 text-white px-4 py-2 rounded">Add New Shift</a>
    </div>
            ...
            <tbody>
                @foreach($shifts as $shift)
                    <tr>
                        ...
                        <td class="py-2 px-4 border-b">{{ $shift->shift_start }}</td>
                        <td class="py-2 px-4 border-b">{{ $shift->shift_end }}</td>
                        <td class="py-2 px-4 border-b">
                            <a href="{{ route('shift.edit', $shift->id) }}" class="text-blue-500">Edit</a>
                            <form action="{{ route('shift.destroy', $shift->id) }}" method="POST" style="display:inline;"onsubmit="return confirm('Are you sure you want to delete this shift?');">
                                @csrf
                                @method('DELETE')
