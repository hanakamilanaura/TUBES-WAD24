...
@section('content')
    <div class="flex items-center mb-4">
        ...
        <h1 class="text-xl font-extrabold text-gray-900">Edit Division</h1>
    </div>
    ...
    <form action="{{ route('division.update', $division->id) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')

        <div>
            <label for="name" class="block text-sm font-medium text-gray-700">Division Name</label>
            <input type="text" id="name" name="name" value="{{ old('name', $division->name) }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-opacity-50" placeholder="Enter division name">
        </div>

        <div>
            <label for="description" class="block text-sm font-medium text-gray-700">Description</label>
            <textarea id="description" name="description" rows="4" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-opacity-50" placeholder="Enter division description">{{ old('description', $division->description) }}</textarea>
        </div>
